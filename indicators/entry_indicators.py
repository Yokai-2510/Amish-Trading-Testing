...
def process_instruments_indicators(r, instruments, data_type, strategy_config):
    ...
    for instrument in instruments:
        ...
        chart_type = strategy_config.get("basic_config", {}).get("entry_config", {}).get("chart_type", "candlestick")
        timeframe = strategy_config.get("basic_config", {}).get("entry_config", {}).get("time_frame", "15m")
        indicators_config = strategy_config.get("entry_indicators", {})
...
def update_entry_indicators():
    ...
    r.jsonset("trading_setup", Path(".indicators.stocks.entry_indicators"), stocks_indicators)
    r.jsonset("trading_setup", Path(".indicators.options.entry_indicators"), options_indicators)
